...
#문제 2 모든 상품내역에서 상품번호, 상품 카테고리명, 상품명, 상품가격, 판매자이름, 판매자 연락처 조회
SELECT 
	`prodNo`,
	`cateName`,
	`prodName`,
...
#문제 3 모든 고객의 아이디, 이름, 휴대폰, 현재 포인트, 적립포인트 총합을 조회하시오. 
#(단 적립포인트 내역이 없으면 0으로 출력)
# if () 함수는 조건이 참이면 첫 번째 값을 반환하고 거짓이면 두 번째 값을 반환
SELECT 
	a.`userId`,
	`userName`,
	`userHp`,
...
#문제5 모든 주문의 주문번호, 주문자아이디, 고객명, 상품명, 주문일자를 조회하시오.
